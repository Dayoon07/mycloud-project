...
<body>
    ...
    <div class="flex flex-col md:flex-row">
        <nav class="w-full lg:w-64 bg-white p-4 mb-4 lg:mb-0 hidden lg:block max-lg:fixed h-full z-20">
		    <ul class="space-y-4">
		        ...
		        <li>
		            <a href="${ cl }/chkmsg" class="flex items-center gap-2 block px-4 py-2 text-gray-700 bg-blue-100 rounded-full">
		                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <main class="flex-1 p-6">
            <c:choose>
                ...
                <c:otherwise>
                	<c:if test="${ empty chkMymsg }">
                		<h1 class="text-2xl">메세지가 없습니다</h1>
                	</c:if>
                	<c:if test="${ not empty chkMymsg }">
	                	<c:forEach var="cmm" items="${ chkMymsg }">
	                		<div style="font-size: 14px;" class="block flex justify-between lg:items-center px-4 py-2 text-md border-b cursor-pointer hover:bg-gray-100" 
	                			data-id="${cmm.msgId}"
							    data-sender="${cmm.sender}"
							    data-receiver="${cmm.receiver}"
							    data-title="${fn:escapeXml(cmm.title)}"
							    data-content="${fn:escapeXml(cmm.content)}"
							    data-datetime="${cmm.datetime}"
							    data-contentfile="${cmm.contentFile}"
							    onclick="particularDisplay(this)">
				                <div class="lg:flex lg:items-center">
				                    <div class="font-semibold">${ cmm.sender }</div>
				                    <div class="lg:px-10 text-gray-500">${ cmm.title }</div>
				                </div>
				                <div>
				                	<c:if test="${ cmm.datetime.substring(0, 4).equals(LocalDateTime.now().format(DateTimeFormatter.ofPattern('yyyy'))) }">
				                		${ cmm.datetime.substring(5, 7) += '월 ' += cmm.datetime.substring(8, 10) += '일' }
				                		${ cmm.datetime.substring(14, 19) }
				                	</c:if>
				                </div>
				            </div>
	                	</c:forEach>
	...
	<script src="${ cl }/source/js/chkmsg.js"></script>
